...
sethandleChange(searchedVal){
  ...
  var a=this.state.data.filter((obj) => 
...
(this.state.type==='jackets')?
 request = await fetch('https://bad-api-assignment.reaktor.com/products/jackets',  {
      cache: "default",
      headers: {
        'x-force-error-mode':'all'
        },
  }):
   request = await fetch('https://bad-api-assignment.reaktor.com/products/shirts',  {
    cache: "default",
    headers: {
      'x-force-error-mode':'all'
      },
...
jkNew.sort( compare );
  ...
  getData().then((data)=>this.setState({'data':data}))//Jackets
   ...
   let datatoDisplay = (this.state.filtered && this.state.filtered.length>0)?this.state.filtered: this.state.data
        return (  
             <div>
